...
public class CustomOrchestrator extends DefaultOrchestrator {
    ...
    /**
     * This function compares the fuzzy output and returns the best action in
     * stage 1
     *
     */
    ...
    /**
     * generate the fuzzy decision tree used in each stage from the
    ...
    /**
     * This function updates Q values and updates the decision tree. the Q table
     * is saved in a file and then used to generate the new tree, in order to
     * get the fuzzy rules, that are used in classification.
     */
    public void reinforcementRecieved(Task task, boolean reinforcement) throws IOException {
        ...
        if (task.getOffloadingDestination().getEnergyModel().isBatteryPowered()) // Discount the reward by 10% if the offloading destination is a battery powered
    ...
    /**
     * Here we update the Q value of a state-action pair according to the
     * received reward
     *
     */
    ...
    protected void checkComputingNode(ComputingNode computingNode) {
        if (computingNode.isSensor()) {
            throw new IllegalArgumentException(
                    getClass().getSimpleName() + " - The forbidden happened! The orchestration algorithm \"" + this.algorithm
